...
type Props = NativeStackScreenProps<EstadisticasStackParamList, "EstadisticasSemanal">;

const todayIso = () => format(new Date(), "yyyy-MM-dd");

export function EstadisticasSemanalScreen({ navigation }: Props) {
  const now = new Date();
      ...
      <ScrollView className="px-5 pt-4" contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View className="flex-row items-center mb-1">
          ...
          <Text className="text-2xl font-bold text-surface-dark dark:text-white">
            {t("estadisticas.weeklyTitle")}
          </Text>
        </View>
        <Text className="text-sm text-gray-500 mb-4">
          {t("estadisticas.weeklyRange", {
            from: format(startOfWeek(now, { weekStartsOn: 1 }), "dd MMM"),
            to: format(endOfWeek(now, { weekStartsOn: 1 }), "dd MMM"),
          })}
        </Text>
        ...
        <Text className="text-lg font-semibold text-surface-dark dark:text-white mb-2 mt-2">
          {t("estadisticas.viewSpecificWeek")}
        </Text>
        <Calendar
          current={todayIso()}
